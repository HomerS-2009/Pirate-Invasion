  ...
  if (boats.length > 0) {
    ...
    for (var i = 0; i < boats.length; i++) {
      if (boats[i]) {
        ...
        if(collision.collided && !boats[i].isBroken){ // boats[i].isBroken === false !false = true, !true = false. if condition executes only if the condition returns TRUE
...
function gameOver(){
  ...
  function(isConfirm){ //anonymous functions - functions with no name
    if(isConfirm){
      location.reload()
    }
  }
